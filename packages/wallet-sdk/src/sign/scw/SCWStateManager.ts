...
export class SCWStateManager {
  ...
  constructor(params: { appChainIds: number[] }) {
    ...
    this._accounts = this.loadItemFromStorage<AddressString[]>(ACCOUNTS_KEY) || [];
    this._activeChain = this.loadItemFromStorage<Chain>(ACTIVE_CHAIN_STORAGE_KEY) || {
      id: params.appChainIds?.[0] ?? 1,
    };
  }
