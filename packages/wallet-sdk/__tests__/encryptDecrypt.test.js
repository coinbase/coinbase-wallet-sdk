/* eslint-disable @typescript-eslint/no-var-requires */
/* eslint-disable jest/no-done-callback */
const { encrypt, decrypt } = require('../build/dist/relay/aes256gcm');
const { randomBytesHex } = require('../build/dist/util');

describe('encryption and decryption tests', () => {
  it('decrypted output matches original input', (done) => {
    void (async function () {
      const secret = randomBytesHex(32);
      const input = 'coinbasewallet:2.0';
      const cipherText = await encrypt(input, secret);

      decrypt(cipherText, secret).subscribe({
        next: (decryptedText) => {
          expect(input).toEqual(decryptedText);
          done();
        },
      });
    })();
  });

  it('decrypt data sample from client', (done) => {
    (function () {
      const cipherText =
        '';
      const secret = 'c356fe708ea7bbf7b1cc9ff9813c32772b6e0d16332da4c031ba9ea88be9b5ed';
      const sampleDataResult =
        '{"type":"WEB3_RESPONSE","id":"791fe0ec3dc3de49","response":{"method":"requestEthereumAccounts","result":["0xdf0635793e91d4f8e7426dbd9ed08471186f428d"]}}';
      decrypt(cipherText, secret).subscribe({
        next: (value) => {
          expect(sampleDataResult).toEqual(value);
          done();
        },
      });
    })();
  });
});
