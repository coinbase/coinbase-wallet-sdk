...
export class WalletSDKRelay extends WalletSDKRelayAbstract {
  ...
  protected readonly diagnosticLogger?: DiagnosticLogger;
  ...
  constructor(options: Readonly<WalletSDKRelayOptions>) {
    ...
    this.subscriptions.add(
      this.connection.sessionConfig$.subscribe({
        ...
        error: () => {
          this.diagnosticLogger?.log(EVENTS.GENERAL_ERROR, {
          ...
          tap((linked: boolean) => {
            ...
            if (cachedAddresses) {
              ...
              if (
                ...
                this.diagnosticLogger?.log(EVENTS.UNLINKED_ERROR_STATE, {
                  sessionIdHash,
                });
              }
            }
    ...
    this.subscriptions.add(
      this.connection.sessionConfig$
        ...
        .subscribe(() => {
          ...
          this.diagnosticLogger?.log(EVENTS.METADATA_DESTROYED, {
            alreadyDestroyed,
    ...
    this.connection
      ...
      .subscribe(
        _ => {
          ...
          this.diagnosticLogger?.log(EVENTS.SESSION_STATE_CHANGE, {
          ...
          if (storedSession?.id === this._session.id) {
            ...
            this.diagnosticLogger?.log(EVENTS.SKIPPED_CLEARING_SESSION, {
              sessionIdHash: this.getSessionIdHash(),
        ...
        (err: string) => {
          this.diagnosticLogger?.log(EVENTS.FAILURE, {
            method: "relay::resetAndReload",
  ...
  private publishWeb3RequestEvent(id: string, request: Web3Request): void {
    ...
    const storedSession = Session.load(this.storage);
    this.diagnosticLogger?.log(EVENTS.WEB3_REQUEST, {
      eventId: message.id,
    ...
    this.subscriptions.add(
      this.publishEvent("Web3Request", message, true).subscribe({
        next: _ => {
          this.diagnosticLogger?.log(EVENTS.WEB3_REQUEST_PUBLISHED, {
            eventId: message.id,
            method: `relay::${message.request.method}`,
            sessionIdHash: this.getSessionIdHash(),
  ...
  private handleWeb3ResponseMessage(message: Web3ResponseMessage) {
    ...
    this.diagnosticLogger?.log(EVENTS.WEB3_RESPONSE, {
      eventId: message.id,
